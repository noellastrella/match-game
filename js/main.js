(()=>{
  ...
  function init(){    
    ...
    for(let i=0; i<=numColumns; i++){
      guessColumns = document.createElement('div');
      newCol.setAttribute('draggable', "true");
      newCol.setAttribute('id', "section-"+(i+1));
      newCol.classList.add("col");
      newCol.classList.add("cols");
      newCol.classList.add("guessColumn");
      document.querySelector("#guess").appendChild(newCol);
    }
  ...
  function createSet(){
    draggables = [];
    let colors = ["yellow", "red", "green", "white", "aqua", "orange", "pink"];
    document.querySelectorAll("#guess .col").forEach(e=>e.innerHTML = "")
    document.querySelectorAll("#answers .col").forEach(e=>e.innerHTML = "")
    document.querySelectorAll("#guess .col").forEach((e,i)=>draggables[i] = createNew(i, colors));
    
    updateScores();
  }

  function createNew(num, colors){
      let randomColor = colors.shift() //"%23" + Math.floor(Math.random()*16777215).toString(16);
...
url('data:image/svg+xml;utf8,<svg width="100px" height="100px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" ><path fill="${randomColor}" d="M14,7.59V4a1,1,0,0,0-1-1H11a1,1,0,0,0-1,1V7.59A3.41,3.41,0,0,1,9,10H9a3.41,3.41,0,0,0-1,2.41V20a1,1,0,0,0,1,1h6a1,1,0,0,0,1-1V12.41A3.41,3.41,0,0,0,15,10h0A3.41,3.41,0,0,1,14,7.59Z" style=" stroke-width: 1;"></path><path d="M14,7.59V4a1,1,0,0,0-1-1H11a1,1,0,0,0-1,1V7.59A3.41,3.41,0,0,1,9,10H9a3.41,3.41,0,0,0-1,2.41V20a1,1,0,0,0,1,1h6a1,1,0,0,0,1-1V12.41A3.41,3.41,0,0,0,15,10h0A3.41,3.41,0,0,1,14,7.59Z" style="fill: none; stroke: rgb(0, 0, 0); stroke-linecap: round; stroke-linejoin: round; stroke-width: .75;"></path><text style="font-size:7px" x="42%" y="70%" fill="black">${num}</text></svg>');`);  
